...
| 数据库 | MySQL 8（库需预先创建，见 `db/schema.sql`） |
...
```bash
# 首次或需要重建表结构时：建库建表（脚本内含 DROP TABLE）
...
- **库必须预先存在**：服务不建库，库不存在会直接启动失败
- 表结构由 JPA（`ddl-auto=update`）自动补建 / 补字段，新增实体字段不必手工改 SQL

**"原来的数据"从哪来**：不是写死在 SQL 里的，而是 `DataInitializer` 用 Java 代码构造后经 JPA 写库。
首次建表后启动会灌入 6 款贷款产品、6 条惠农资讯、3 个账号（`farmer` / `coop` / `auditor`）、
12 张覆盖全生命周期的申请单与 8 条信用积分日志；之后就交给管理员通过 `/api/admin/**`
与新增/修改接口维护，代码不再参与。
...
> 注意：MySQL 是**持久库**，数据跨重启保留，种子数据仅在表为空时灌入一次。
> 连续多次运行「申请 → 放款 → 结清」闭环会让信用积分累积上涨。
> 想回到初始状态：重跑 `mysql -uroot -p < db/schema.sql`（会清空这 7 张表）后重启后端。
...
├── db/schema.sql                 # MySQL 建库建表脚本（首次启动前执行）
...
└── src/main
    ...
    └── resources
        ...
        └── application-mysql.yml # MySQL 数据源（连接信息）
```
